...
export default (app, router, auth, logger) => {
        ...
        var menu_list = menus.map(mnu => {
            return new Promise((resolve, reject) => {
                Role.find()
                    ...
                    .then(status => {
                        return Menu.remove({ _id: mnu._id })
                    })
                    .then(menu => {
                        return Menu.find({ parent: mnu._id })
                    })
                    .then(childMenus => {
    ...
    function deleteMenuRefFromRole(roles, menu_id) {
        var data_roles = roles.map(role => {
            return new Promise((resolve, reject) => {
                Role.update({ _id: role._id }, { $pull: { 'menu': { _id: menu_id }, 'full_generation': menu_id } })
                    .exec((err, restRole) => {
                        if (err) {
                            reject(err);
                        }
